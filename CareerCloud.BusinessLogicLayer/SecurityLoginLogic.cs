	...
	public class SecurityLoginLogic : BaseLogic<SecurityLoginPoco>
	{
		...
		protected override void Verify(SecurityLoginPoco[] pocos)
		{
			...
			foreach (var entity in pocos)
			{
				...
				{
					exceptions.Add(new ValidationException(700, $"Password for SecurityLogin {entity.Id} cannot be null"));
				}
				else if (entity.Password.Length < 10)
				{
					exceptions.Add(new ValidationException(700, $"Password for SecurityLogin {entity.Id} must be at least 10 characters."));
				}
				else if (!requiredExtendedPasswordChars.Any(t => entity.Password.Contains(t)))
				{
					exceptions.Add(new ValidationException(701, $"Password for SecurityLogin {entity.Id} must contain an extended character of '$', '*', '#', '_' or '@' ."));
				}

				if (string.IsNullOrEmpty(entity.PhoneNumber))
				{
					exceptions.Add(new ValidationException(702, $"PhoneNumber for SecurityLogin {entity.Id} is required"));
				}
				else
				{
					string[] phoneComponents = entity.PhoneNumber.Split('-');
					if (phoneComponents.Length < 3)
					{
						exceptions.Add(new ValidationException(703, $"PhoneNumber for SecurityLogin {entity.Id} is not in the required format."));
					}
					else
					{
				...
				else if (!Regex.IsMatch(entity.EmailAddress, @"\A(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)\Z", RegexOptions.IgnoreCase))
				{
					exceptions.Add(new ValidationException(704, "EmailAddress for SecurityLogin {poco.Id} is not a valid email address format."));
				}
